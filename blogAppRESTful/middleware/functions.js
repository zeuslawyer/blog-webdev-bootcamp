...
middleware.isUserAuthenticated = (req, res, next) => {
  ...
  console.log(
    "user not signed in --redirected to login page, and then returnPath may be used"
  );
  res.redirect("/login");
};
...
middleware.checkBlogAuthor = (req, res, next) => {
  Blog.findById(req.params.id, (err, retrievedBlog) => {
    if (err) {
      res.send(
        " DB retrieve for Blog object didnt work. Could not verify user and author"
      );
    } else {
      // e stablish if authenticated user is authorised to edit/delete etc
      if (
        res.locals.adminLoggedIn ||
        (retrievedBlog.author.id &&
          retrievedBlog.author.id.equals(req.user._id))
      ) {
        ...
        console.log(
          ...
          }, you are not authorised to do this to ${retrievedBlog}.`
        );
        res.redirect("back");
      }
    }
...
middleware.checkCommentAuthor = (req, res, next) => {
  Comment.findById(req.params.commId, (err, retrievedComment) => {
    if (err) {
      res.send(
        " DB retrieve for Comment object didnt work. Could not verify user and authorship"
      );
    } else {
...
middleware.viewsData = (req, res, next) => {
  ...
  if (req.user) {
    //if someone logged in, check if admin or not
    res.locals.adminLoggedIn = req.user.username === "zp@zp.com" ? true : false;
  }
  console.log("is admin logged in?  " + !!req.user);
